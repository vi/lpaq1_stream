...
private:
  ...
  int code(int y=0) {
    ...
    if(autonomousmode) {
        ...
        if(autonomous_mood) {
            --autonomous_mood;
            predictor.update(random()>RAND_MAX/2);
        } else {
            predictor.update(p>2048);
        }

        if (random()<RAND_MAX/2000) {
            autonomous_mood = random()*100LL/RAND_MAX;
        }
...
int main(int argc, char **argv) {
  ...
  // Decompress
    ...
    while (size-->0 || streammode) {
      ...
      if(feof(in) && !signal_workaround)break;
      signal_workaround=0;
    }
    if(argv[1][0]=='a') {
        autonomousmode=1;
